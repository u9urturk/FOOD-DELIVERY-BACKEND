...
export class RealtimeAuthService {
    ...
    authenticate(client: Socket): { userId: string; sessionId: string } {
        // Yeni mimari: access token cookie'de değil. Authorization header veya handshake.auth / query üzerinden gelir.
        ...
        // Socket.IO client -> io(url, { auth: { accessToken } }) veya auth: { token }
        if (!token) {
    ...
    // Secret rotation support: env JWT_SECRETS="kid1=secret1,kid2=secret2"; falls back to JWT_SECRET when kid unknown/absent
    ...
    private buildSecretsMap(): Record<string, string> {
        ...
        const raw = this.config.get<string>('JWT_SECRETS');
        if (raw) {
        ...
        // Add default secret under implicit key 'default'
